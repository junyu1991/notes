...
下图蓝色区域表示了各个对象生命周期的分布区域，X轴是以分配的字节为单位的生命周期，Y轴是相应生存周期对应的对象中的字节数。从图中可看出大部分空间在被分配后都会很快的被回收，比如很多在循环中的对象都只会存活一个循环。
...
- 年轻代中主要是新生成的对象以及等待老化(被移动到老年代)的对象。年轻代由一个eden以及两个survivor空间组成。新生成的对象都是被放在eden空间中的，当eden空间被填满时，就会触发一次小垃圾回收(minor garbage collection)，minor gc只针对于年轻代。年轻代中的对象大多数生命周期都很很短，少部分生命周期较长的对象会被移动到老年代中。所有的minor gc都会执行**“停止世界”**事件，即当执行minor gc时，所有的应用线程都会被暂停直到minor gc操作完成。
- 老年代中主要是生命周期较长的对象。通常情况下，对于年轻代中的对象都会设置一个年龄阈值，当年轻代中的对象生命周期达到了该阈值，将该对象则会被移动到老年代中。老年代也需要进行gc操作，该操作被称作“大型垃圾回收”（major garbage collection）。major gc同样也是“停止世界”事件，但是该操作执行频率较低，因为老年代中大部分都是生命周期较长的对象。因此对于追求应用响应速度的应用，应该要尽量减少major gc。major gc执行的暂停时间主要受垃圾回收算法的影响。
...
主要在两方面对垃圾回收算法的指标进行衡量：
- 吞吐量，吞吐量是指未花费在gc上的时间占的总时间的百分比。吞吐量还包括了分配花费的时间。
- 延迟，延迟指的是应用的响应能力。gc暂停应用的操作会影响应用的响应能力。
...
> 使用```-verbose:gc```参数可在日志中查看每次gc时堆的信息。使用效果如下：
```
