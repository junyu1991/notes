...
## shell传参注意点
1. **\*** 参数处理
使用 **\*** 传参，当向shell中传递 **\*** 号参数时需要注意，如果直接使用 **\*** ，shell中拿到的参数会包含多个值，如以下脚本:
``` sh
#!/bin/bash
if [ -n $1 ]
then
...
使用方式为：./test_param.sh -b "\*.bak"，此时就可以使用"或者'包围参数值或者转义使用(如：\\\*.bak)，若没有引号包围，结果和不使用此方案结果一样，会出现参数不正确的错误。
